...
payload_put = {
    # TODO: Can't change the company_name
...
def test_query_all_companies(client, db_session, sample_company):
    # Get /api/companies
    ...
    assert any(
        company_data["company_name"] == sample_company.company_name
...
def test_query_all_companies_true(client, db_session, sample_company):
...
def test_query_all_companies_false(client, db_session, sample_company):
...
def test_query_all_companies_empty(client, db_session):
    # Get /api/companies
    response = client.get("/api/companies")
    assert response.status_code == 200
    data = response.get_json()
...
# TODO: sample_company.company_name not working
def test_query_company(client, db_session, sample_company):
...
def test_create_company(client, db_session, sample_company):
    # Post /api/companies
...
def test_create_company_duplicate(client, db_session, sample_company):
    # Post /api/companies
    response = client.post("/api/companies", json=payload_create)
    response = client.post("/api/companies", json=payload_create)
...
# TODO: sample_company.company_name not working
def test_update_company(client, db_session, sample_company):
    # Put /api/companies/<company_name>
    company_name = "Küche"
    response = client.put(f"/api/companies/{company_name}", json=payload_put)
    assert response.status_code == 200
    data = response.get_json()
    assert isinstance(data, dict)
    assert len(data) == 8
    ...
    # response = client.get(f"/api/companies/Kitchen")
    # assert response2.status_code == 200
    # data2 = response2.get_json()
    # assert isinstance(data2, dict)
    # assert len(data2) == 8
    # assert data2["company_name"] == payload_put["company_name"]  #sample_company.company_name


# TODO: sample_company.company_name not working
def test_delete_company(client, db_session, sample_company):
    # Delete /api/companies/<company_name>
    company_name = "Küche"
    response = client.delete(f"/api/companies/{company_name}")
    assert response.status_code == 200
    data = response.get_json()
    assert isinstance(data, dict)
    response = client.get(f"/api/companies/{company_name}")
