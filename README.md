...
- **`[hourly_pay]`** — Village-wide pay tuning: `increase` is an integer added to each company’s stored hourly pay. Company JSON returns **`hourly_pay`: `{ "base", "increase", "total" }`** so clients can show the combined rate and see the village bump separately; other keys (e.g. `tax`) are passed through to clients via **`GET /api/village-data`** if you define them.
