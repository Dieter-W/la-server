...
def hash_password(password: str) -> str:
    ...
    return generate_password_hash(password.lower())
...
def verify_password(stored_hash: str, plain_password: str) -> bool:
    ...
    return check_password_hash(stored_hash, plain_password.lower())
...
def create_refresh_token(identity, additional_claims=None):
    """Create a refresh token for the given identity and additional claims."""
    if additional_claims is None:
        return jwt_create_refresh_token(identity=str(identity))
